...
integrate this code to my given below full main code 
...
def plot_measurements(tracks, ax, plot_type, selected_track_ids=None):
    ...
    for track in tracks:
        ...
        if plot_type == "Range vs Time":
            ...
            ax.scatter(times, Sf_x, label=f'Track {track["track_id"]} Sf X', linestyle='--')
...
class KalmanFilterGUI(QWidget):
    ...
    def update_selected_tracks(self):
        ...
        for i in range(self.track_selection_layout_inner.count()):
            checkbox = self.track_selection_layout_inner.itemAt(i).widget()
